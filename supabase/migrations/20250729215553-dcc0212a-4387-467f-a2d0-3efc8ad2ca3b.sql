...
-- Note: application_status uses permit_status_enum type
...
-- Add foreign key for assigned reviewer (already exists in table)
-- ALTER TABLE public.permit_applications 
...
BEGIN
  ...
  IF NEW.application_status IN ('submitted', 'under_review') AND NEW.assigned_reviewer_id IS NULL THEN
...
CREATE TABLE IF NOT EXISTS public.permit_notifications (
  ...
  CONSTRAINT fk_permit_notifications_permit 
    FOREIGN KEY (permit_id) REFERENCES public.permit_applications(permit_id),
