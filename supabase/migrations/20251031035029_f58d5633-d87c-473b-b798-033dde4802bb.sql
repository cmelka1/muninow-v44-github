-- Fix all table and column name errors in create_communication_notification function
-- This corrects business_license, service_application, and tax_submission blocks
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'permit_review_comments' THEN
    ...
    -- Get permit details including applicant_id and permit number
    SELECT 
      pa.applicant_id,
      pa.permit_number,
      jsonb_build_object(
  ...
  ELSIF TG_TABLE_NAME = 'business_license_comments' THEN
    ...
    -- Get business license details - FIXED: Use correct table and column names
    SELECT 
      bla.user_id,
      bla.license_number,
      jsonb_build_object(
  ...
  ELSIF TG_TABLE_NAME = 'municipal_service_application_comments' THEN
    ...
    -- Get service application details - FIXED: Use correct column names
    SELECT 
      sa.user_id,
  ...
  ELSIF TG_TABLE_NAME = 'tax_submission_comments' THEN
    ...
    -- Get tax submission details - FIXED: Use correct column names and no unnecessary join
    SELECT 
      ts.user_id,
      ts.id::TEXT,
      jsonb_build_object(
