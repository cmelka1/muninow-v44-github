-- Fix the create_service_application_renewal function to correctly calculate days until expiration
-- The issue was using EXTRACT on date subtraction which already returns an integer

CREATE OR REPLACE FUNCTION public.create_service_application_renewal(p_original_application_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Calculate days until expiration (date subtraction returns integer directly)
