-- Update the get_user_bill_summary_for_municipal function to count only unpaid bills
-- and calculate total amount due for unpaid bills only
...
BEGIN
  ...
  -- Return user profile with unpaid bill summary
  RETURN QUERY
  SELECT 
    ...
    profiles.created_at,
    profiles.updated_at,
    COALESCE(unpaid_bill_summary.unpaid_bill_count, 0) as unpaid_bill_count,
    COALESCE(unpaid_bill_summary.total_amount_due_cents::bigint, 0) as total_amount_due_cents,
    COALESCE(unpaid_bill_summary.unpaid_bill_count, 0) > 0 as has_bills
  FROM public.profiles
  LEFT JOIN (
    SELECT 
      mb.user_id,
    ...
    WHERE mb.customer_id = requesting_customer_id
      ...
      AND (mb.payment_status != 'paid' OR mb.payment_status IS NULL)
    GROUP BY mb.user_id
  ) unpaid_bill_summary ON profiles.id = unpaid_bill_summary.user_id
