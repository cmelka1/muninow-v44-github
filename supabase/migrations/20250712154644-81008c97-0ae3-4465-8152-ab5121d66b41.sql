-- Create 30 master bills with full data relationships (final version)
...
random_categories AS (
  SELECT 
    ...
    CASE (RANDOM() * 13)::int
      WHEN 0 THEN 'Utilities & Services'
      WHEN 1 THEN 'Utilities & Services'
      WHEN 2 THEN 'Utilities & Services'
      WHEN 3 THEN 'Property-Related'
      WHEN 4 THEN 'Property-Related'
      WHEN 5 THEN 'Vehicle & Transportation'
      WHEN 6 THEN 'Vehicle & Transportation'
      WHEN 7 THEN 'Licensing & Registration'
      WHEN 8 THEN 'Administrative & Civic Fees'
      WHEN 9 THEN 'Court & Legal'
      WHEN 10 THEN 'Community Programs & Education'
      WHEN 11 THEN 'Other Specialized Payments'
      ELSE 'Other'
    END as category,
    CASE (RANDOM() * 13)::int
      WHEN 0 THEN 'Water'
      ...
      WHEN 3 THEN 'Property Taxes'
      WHEN 4 THEN 'Special Assessments'
      WHEN 5 THEN 'Parking Tickets'
      WHEN 6 THEN 'Traffic Fines'
      WHEN 7 THEN 'Business Licenses'
      WHEN 8 THEN 'Public Records Requests (FOIA)'
      WHEN 9 THEN 'Court Fines'
      WHEN 10 THEN 'Recreation Program Fees'
      WHEN 11 THEN 'Impact Fees'
      ELSE 'Other'
    END as subcategory
  FROM bill_series
)
...
SELECT 
  ...
  'EXT-' || LPAD(rc.bill_number::text, 6, '0') as external_bill_number,
  ...
  -- Late fees (total will be auto-calculated)
  ...
  rc.assigned_merchant_id as merchant_id,
...
LEFT JOIN merchant_fee_profiles mfp ON mfp.merchant_id = rc.assigned_merchant_id;